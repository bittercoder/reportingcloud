...
namespace ReportingCloud.Designer
{
	...
	internal class DesignerUtility
	{
        ...
        static internal bool GetSharedConnectionInfo(ReportDesigner dsr, string filename, out string dataProvider, out string connectInfo)
        {
            dataProvider = null;
            connectInfo = null;
            ...
            try
            {
                ...
                xml = ReportingCloud.Engine.DataSourceReference.Retrieve(filename, pswd);
            }
            catch
            {
		...
		static internal void GetSqlData(string dataProvider, string connection, string sql, IList parameters, DataTable dt)
		{
			...
			try
			{
				...
				cnSQL = EngineConfig.GetConnection(dataProvider, connection);
				if (cnSQL == null)
					return;
...
//                        rowValues[ci++] = val;
        ...
        static internal bool GetConnnectionInfo(DesignXmlDraw d, string ds, out string dataProvider, out string connection)
        {
            ...
            if (dataSourceReference != null)
            {
                ...
                Control p = d;
                MDIChild mc = null;
                while (p != null && !(p is ReportDesigner))
                {
                    if (p is MDIChild)
                        mc = (MDIChild)p;
                ...
                if (!DesignerUtility.GetSharedConnectionInfo((ReportDesigner)p, filename, out dataProvider, out connection))
                    return false;
            }
            else
            {
        ...
        static internal List<SqlSchemaInfo> GetSchemaInfo(string dataProvider, string connection)
		{
			...
			try
			{
				...
				string sql = EngineConfig.GetTableSelect(dataProvider, cnSQL);
