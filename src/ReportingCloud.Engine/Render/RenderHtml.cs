...
namespace ReportingCloud.Engine
{
    ...
    {
        readonly IStreamGen _sg; // stream generater
        readonly Hashtable _styles; // hash table of styles we've generated
        readonly Report r; // report
        readonly StringWriter tw; // temporary location where the output is going
        bool _Asp; // denotes ASP.NET compatible HTML; e.g. no <html>, <body>
        ...
        int _SkipMatrixCols; // # of matrix columns to skip
        Bitmap _bm; // bm and
        Graphics _g; //		  g are needed when calculating string heights
        bool bScriptTableSort; // need to generate table sort javascript in header
        bool bScriptToggle; // need to generate toggle javascript in header
        ...
        {
            if (tb.IsHtml(r, row)) // we leave the text as is (except to handle unicode) when request is to treat as html
            {
            ...
            else
            {
                ...
                t = Bookmark(tb.BookmarkValue(r, row), t);

                // handle any specified actions
                t = Action(tb.Action, row, t, tb.ToolTipValue(r, row));
            }
        ...
        public bool TableStart(Table t, Row row)
        {
            ...
            {
                bScriptTableSort = true;
            }

            string bookmark = t.BookmarkValue(r, row);
            if (bookmark != null)
                tw.WriteLine("<div id=\"{0}\">", bookmark); // can't use the table id since we're using for css style

            // Calculate the width of all the columns
            int width = t.WidthInPixels(r, row);
            if (width <= 0)
        ...
        public void TableCellStart(TableCell t, Row row)
        {
            ...
            if (t.InTableHeader && t.OwnerTable.TableColumns != null)
            {
                ...
                {
                    TableColumn tc = t.OwnerTable.TableColumns.Items[ci];
        ...
        {
            string cellType = t.InTableHeader ? "th" : "td";
            var tb = t.ReportItems.Items[0] as Textbox;
        ...
        public void Image(Image i, Row r, string mimeType, Stream ioin)
        {
            ...
            try
            {
                ...
                {
                    var ba = new byte[ioin.Length];
                    ioin.Read(ba, 0, ba.Length);
                    io.Write(ba, 0, ba.Length);
                }
                else
                {
                    var ba = new byte[1000]; // read a 1000 bytes at a time
                    while (true)
                    {
        ...
        public void Line(Line l, Row r)
        {
            ...
            if (l.Height == null || l.Height.PixelsY > 0) // only handle horizontal rule
            {
                ...
                bVertical = true;
                t =
                    "<TABLE style=\"border-collapse:collapse;BORDER-STYLE: none;WIDTH: {0}; POSITION: absolute; LEFT: {1}; TOP: {2}; HEIGHT: {3}; BACKGROUND-COLOR:{4};\"><TBODY><TR style=\"WIDTH:{0}\"><TD style=\"WIDTH:{0}\"></TD></TR></TBODY></TABLE>";
            }
            else
            {
                bVertical = false;
                t =
                    "<TABLE style=\"border-collapse:collapse;BORDER-STYLE: none;WIDTH: {0}; POSITION: absolute; LEFT: {1}; TOP: {2}; HEIGHT: {3}; BACKGROUND-COLOR:{4};\"><TBODY><TR style=\"HEIGHT:{3}\"><TD style=\"HEIGHT:{3}\"></TD></TR></TBODY></TABLE>";
            }
        ...
        public void Subreport(Subreport s, Row r)
        {
            ...
            tw.WriteLine("<div class='{0}'><table>", cssName);

            s.ReportDefn.Run(this);

            tw.WriteLine("</table></div>");
        }
        ...
        void ScriptGenerate(TextWriter ftw)
        {
...
");
        ...
        string Action(Action a, Row r, string t, string tooltip)
        {
            if (a == null)
                return t;
            ...
            else if (a.Drill != null)
            {
                ...
                var args = new StringBuilder("<a target=\"_top\" href=\"");
        ...
        string Bookmark(string bm, string t)
        {
            if (bm == null)
                return t;
        ...
        void CssGenerate(TextWriter ftw)
        {
            if (_styles.Count <= 0)
                return;
        ...
        string CssPosition(ReportLink rl, Row row, bool bForceRelative, float h, float w)
        {
            ...
            {
                sb.AppendFormat(NumberFormatInfo.InvariantInfo, "top: {0}pt; ", ri.Gap(r));
            }
            if (ri is List)
            {
                var l = ri as List;
                sb.AppendFormat(NumberFormatInfo.InvariantInfo, "height: {0}pt; ", l.HeightOfList(r, GetGraphics, row));
            }
        ...
        string SortType(TableCell tc, Textbox tb)
        {
            ...
            if (d == null)
                return null;
            TableRow tr = d.TableRows.Items[0];
            if (tr == null)
                return null;
            TableCell dtc = tr.TableCells.Items[tc.ColIndex];
            if (dtc == null)
                return null;
            var dtb = dtc.ReportItems.Items[0] as Textbox;
            if (dtb == null)
                return null;
