...
namespace ReportingCloud.Engine
{
    ...
    {
        readonly MatrixCellEntry[,] _DataDefn;
        ...
        public MemoryStream _aStream; // GJL 14082008 Using Vector Graphics
        ...
        internal Metafile Image(Report rpt) // GJL 14082008 Using Vector Graphics
        {
            if (_bm == null)
                Draw(rpt);
        ...
        {
            var rect = new System.Drawing.Rectangle(0, 0, Layout.Width, Layout.Height);
        ...
        protected System.Drawing.Rectangle DrawLegend(Report rpt, Graphics g, bool bMarker, bool bBeforePlotDrawn)
        {
            ...
            try // no matter what we want to dispose of the graphic resources
            {
                ...
                var boxSize = (int) (maxTextHeight*.8);
                ...
                int saveX = x;
                ChartMarkerEnum cm = ChartDefn.Type == ChartTypeEnum.Bubble ? ChartMarkerEnum.Bubble : ChartMarkerEnum.None;
                for (int iCol = 1; iCol <= SeriesCount; iCol++)
                {
                     ...
                     * 06122007AJM */
                        ...
                        if ((bMarker || isLine) || (ChartDefn.Type == ChartTypeEnum.Scatter && t == typeof (HatchBrush)))
                            cm = SeriesMarker[iCol - 1];
                        if (isLine && ChartDefn.Type == ChartTypeEnum.Scatter)
                        {
                        ...
                        switch (l.Layout)
                        {
                            case LegendLayoutEnum.Row:
                                ...
                                if (c != "") //14052008WRP to cater for empty strings in the legend
                                {
                                    ...
                                    if ((cm != ChartMarkerEnum.None || ChartDefn.Type == ChartTypeEnum.Scatter) && (t == typeof (HatchBrush))) //GJL 110208 - Don't draw pattern for lines or Bubbles
                                    {
                                        var hb = (HatchBrush) GetSeriesBrush(rpt, 1, iCol);
                            ...
                            case LegendLayoutEnum.Table:
                                ...
                                if (cm != ChartMarkerEnum.None && (t == typeof (HatchBrush))) //GJL 110208 - Don't draw pattern for lines
                                {
                                    var hb = (HatchBrush) GetSeriesBrush(rpt, 1, iCol);
                                    b = new SolidBrush(hb.ForegroundColor);
        ...
        void DrawLegendBox(Graphics g, Brush b, ChartMarkerEnum marker, int x, int y, int boxSize, int intLineSize)
        {
            ...
            try
            {
                if (marker < ChartMarkerEnum.Count)
                {
                    ...
                    if (ChartDefn.Type != ChartTypeEnum.Scatter)
                    {
        ...
        {
            var sizes = new Size[SeriesCount];
        ...
        protected object GetCategoryValue(Report rpt, int row, out TypeCode tc)
        {
            ...
            Row lrow;
            _ChartDefn.ChartMatrix.SetMyData(rpt, mce.Data); // Must set this for evaluation
            if (mce.Data.Data.Count > 0)
                lrow = mce.Data.Data[0];
            else
                lrow = null;
            var ce = (ChartExpression) (mce.DisplayItem);
...
// 20022008 AJM GJL
            ...
            for (int i = 0; i < SeriesCount; i++)
            {
                ...
                switch (ChartPalette.GetStyle(ChartDefn.Palette.EvaluateString(rpt, row), defn.rl))
                {
                    ...
                    case ChartPaletteEnum.SemiTransparent:
                        b[i] = GetSeriesBrushVibrant(i);
                        break;
                    case ChartPaletteEnum.Patterned:
                        b[i] = GetSeriesBrushesPatterned(i);
                        break;
                    case ChartPaletteEnum.PatternedBlack:
                        b[i] = GetSeriesBrushesPatternedBlack(i);
                        break;
                    case ChartPaletteEnum.Custom:
                        b[i] = new SolidBrush(Color.FromName(getColour(rpt, 1, i + 1)));
                        break;
                    default:
                        b[i] = GetSeriesBrushesExcel(i);
                        break;
        ...
        Brush GetSeriesBrushVibrant(int i)
        {
            int alpha = 128;
            Brush result;
            switch (i%10)
...
// 20022008 AJM GJL
        ...
        {
            var m = new ChartMarkerEnum[SeriesCount];
...
// 20022008 AJM GJL
        ...
        protected void AdjustMargins(System.Drawing.Rectangle legendRect, Report rpt, Graphics g)
        {
            ...
            if (Layout.RightMargin < min + _LastCategoryWidth/2)
                Layout.RightMargin = min + _LastCategoryWidth/2;
