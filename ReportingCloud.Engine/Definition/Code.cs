...
//						tsw.Close();
            ...
            if (EngineConfig.DirectoryLoadedFrom == null) {
                if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "ReportingCloud.Engine.dll"))
                {
                    re = AppDomain.CurrentDomain.BaseDirectory + "ReportingCloud.Engine.dll";   // this can fail especially in web scenarios
                }
                else
                {
                    re = AppDomain.CurrentDomain.BaseDirectory + "Bin\\ReportingCloud.Engine.dll";   // this can work especially in web scenarios
                }
            }
            else
                re = EngineConfig.DirectoryLoadedFrom + "ReportingCloud.Engine.dll";     // use EngineConfig.xml directory when available
		...
		internal Type CodeType()
		{
			...
			try
			{
                object instance = _Assembly.CreateInstance("ReportingCloud.vbgen." + this._Classname, false); 
				t = instance.GetType();
			}
			catch (Exception e)
			{
		...
		internal object Load(Report rpt)
		{
			...
			try
			{
                ...
                args[0] = rpt;
                wc.Instance = _Assembly.CreateInstance("ReportingCloud.vbgen." + this._Classname, false, BindingFlags.CreateInstance, null, args, null, null); 
			}
			catch (Exception e)
			{
				wc.Instance = null;
